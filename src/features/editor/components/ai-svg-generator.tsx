...
export const AiSvgGenerator = ({ editor, onClose }: AiSvgGeneratorProps) => {
  ...
  const generateSVG = async () => {
    ...
    try {
      ...
      // Check if we have direct svg_code in the response
      if (data.svg_code) {
      ...
      // Otherwise, try to extract SVG from the messages
        ...
        if (assistantMessages.length > 0) {
          ...
          // Try to extract SVG code from the message
          if (lastAssistantMessage.includes('```svg')) {
            const svgStart = lastAssistantMessage.indexOf('```svg') + 6;
            const svgEnd = lastAssistantMessage.indexOf('```', svgStart);
            if (svgEnd > svgStart) {
              svgCode = lastAssistantMessage.substring(svgStart, svgEnd).trim();
            }
          }
        }
      }
      ...
      const cleanedSvg = processReceivedSVG(svgCode);
